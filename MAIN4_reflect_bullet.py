...
class Bullet:
    def __init__(self, x, y, dx, dy, bullet_type="red"):
        ...
        self.bounce_left = 1 if bullet_type == "bounce" else 0
    ...
    def draw(self, surface, ox, oy):
        bullet_colors = {"red": COLORS["빨강"], "blue": COLORS["파랑"], "bounce": (170, 255, 80)}
        color = bullet_colors.get(self.type, COLORS["화이트"])
        pygame.draw.circle(
            surface,
            color,
...
class AmmoItem:
    ...
    def draw(self, surface, ox, oy):
        ...
        ammo_colors = {"red": COLORS["빨강"], "blue": COLORS["파랑"], "bounce": (170, 255, 80)}
        ammo_labels = {"red": "R", "blue": "B", "bounce": "G"}
        bg_color = ammo_colors.get(self.type, COLORS["화이트"])
        label = ammo_labels.get(self.type, "?")
        text_color = COLORS["블랙"] if self.type == "bounce" else COLORS["화이트"]
...
class GameScene:
    ...
    def _place_items_traps(self):
        ...
        if self.difficulty != "Easy":
            ammo_count = 6
            ammo_types = ["red", "blue", "bounce"]
            for i in range(ammo_count):
                x, y = self._random_floor_pos(exclude_start=True)
                self.ammo_items.append(AmmoItem(x, y, ammo_type=ammo_types[i % len(ammo_types)]))
    ...
    def _check_collisions(self):
        ...
        for ammo in self.ammo_items:
            ...
            if abs(p.x - ammo.x) < pr + ammo.size and abs(p.y - ammo.y) < pr + ammo.size:
                ...
                elif ammo.type == "bounce":
                    p.ammo_bounce += 1
                    ammo.alive = False
                    self._show_msg("🟢 반사 탄약(G) +1 획득!")
    ...
    def shoot_bullet(self, bullet_type, dx=0, dy=0):
        ...
        elif bullet_type == "bounce":
            if p.ammo_bounce <= 0:
                self._show_msg("❌ 반사 탄약 부족")
                return
            p.ammo_bounce -= 1
            dx, dy = p.last_dx, p.last_dy
            if dx == 0 and dy == 0:
                dx = 1
            self.bullets.append(Bullet(p.x + dx*15, p.y + dy*15, dx, dy, "bounce"))
            if "gun" in SOUNDS: SOUNDS["gun"].play()
            self._show_msg("🟢 반사 탄환 발사! 벽에 한 번 튕깁니다")
            self.shoot_cooldown = 15
    ...
    def update(self, keys):
        ...
        for bullet in self.bullets:
            ...
            if (0 <= row < self.ROWS and 0 <= col < self.COLS):
                if self.maze[row][col] == 1:
                    ...
                    elif bullet.type == "bounce" and bullet.bounce_left > 0:
                        prev_col = int(bullet.prev_x // self.TILE)
                        prev_row = int(bullet.prev_y // self.TILE)
                        if 0 <= prev_row < self.ROWS and self.maze[prev_row][col] == 0:
                            bullet.dy *= -1
                        elif 0 <= prev_col < self.COLS and self.maze[row][prev_col] == 0:
                            bullet.dx *= -1
                        else:
                            bullet.dx *= -1
                            bullet.dy *= -1
                        bullet.x, bullet.y = bullet.prev_x, bullet.prev_y
                        bullet.bounce_left -= 1
                    else:
                        bullet.alive = False
        ...
        if self.minimap_timer > 0:
            self.minimap_timer -= 1
    ...
    def draw_minimap(self, surface, x, y, w=136, h=136):
        if self.minimap_timer <= 0:
            return
        ...
        map_x, map_y = x + 8, y + 25
        map_w, map_h = w - 16, h - 33
        scale = min(map_w / self.COLS, map_h / self.ROWS)
        cell = max(1, int(scale))

        for r in range(self.ROWS):
            for c in range(self.COLS):
                if self.maze[r][c] == 1:
                    pygame.draw.rect(surface, COLORS["벽"], (int(map_x + c * scale), int(map_y + r * scale), cell, cell))

        ex = int(map_x + self.exit_col * scale)
        ey = int(map_y + self.exit_row * scale)
        pygame.draw.rect(surface, COLORS["출구"], (ex, ey, max(3, cell + 2), max(3, cell + 2)))

        if self.need_key and not self.player.has_key:
            kx = int(map_x + (self.key_pos[0] / self.TILE) * scale)
            ky = int(map_y + (self.key_pos[1] / self.TILE) * scale)
            pygame.draw.circle(surface, COLORS["열쇠"], (kx, ky), 4)
            key_label = FONT_TINY.render("K", True, COLORS["블랙"])
            surface.blit(key_label, key_label.get_rect(center=(kx, ky)))

        for m in self.monsters:
            if m.alive:
                mx = int(map_x + (m.x / self.TILE) * scale)
    ...
    def draw(self, surface):
        ...
        ammo_text = FONT_TINY.render(f"일반(WASD): {self.player.ammo_red} | 특수(Space): {self.player.ammo_blue} | 반사(E): {self.player.ammo_bounce}", True, COLORS["연그레이"])
            ...
            if event.type == pygame.KEYDOWN:
                if self.scene.difficulty != "Easy":
                    ...
                    if event.key in shoot_keys:
                        self.scene.shoot_bullet(*shoot_keys[event.key])
                    elif event.key == pygame.K_SPACE:
                        self.scene.shoot_bullet("blue")
                    elif event.key == pygame.K_e:
                        self.scene.shoot_bullet("bounce")
    ...
    def draw(self, surface):
        ...
        if self.state == "PLAY" and self.scene:
            ...
            left_lines = [
                ...
                ("- 스페이스바 : 파괴 총알", COLORS["화이트"]), ("- E : 반사 총알", COLORS["화이트"]), ("적", COLORS["민트"]), ("- 충돌 시 체력 감소", COLORS["화이트"])
            ]
            ...
            item_lines = ["아이템", "- 체력 회복", "- 적 제거", "- 속도 증가", "- 시야 확대", "- 적 정지", "- 시간 왜곡", "- 미니맵"]
